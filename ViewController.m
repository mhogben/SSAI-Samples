...
//add these values based on your player
static NSString *kViewControllerAccountId = accountID;
static NSString *kViewControllerPolicyKey = policyKey;
static NSString *kVideoViewControllerVideoId = @videoID;
static NSString *kSsaiAdConfig = adConfigID;
...
{
    NSDictionary *queryParameters = @{@"ad_config_id" : kSsaiAdConfig};
    NSString *vmapUrlQueryParameters = @"&appId=p8&platformType=app&deviceType=ios&cmsId=2464276";
    
    self.playbackService = [[BCOVPlaybackService alloc] initWithAccountId:kViewControllerAccountId
                                                                policyKey:kViewControllerPolicyKey];
    
    [self.playbackService findVideoWithVideoID:kVideoViewControllerVideoId parameters:queryParameters completion:^(BCOVVideo *video, NSDictionary *jsonResponse, NSError *error) {
        if(jsonResponse){
            NSMutableDictionary *updatedJsonResponseDict = [[NSMutableDictionary alloc] init];
            [updatedJsonResponseDict addEntriesFromDictionary:jsonResponse];
            NSMutableArray *updatedJsonResponseSources = [[NSMutableArray alloc] init];
            
            for(NSDictionary *src in updatedJsonResponseDict[@"sources"]){
                NSMutableDictionary *mutableSrc = [[NSMutableDictionary alloc] init];
                [mutableSrc addEntriesFromDictionary:src];
                NSString *vmapUrl = mutableSrc[@"vmap"];
                mutableSrc[@"vmap"] = [vmapUrl stringByAppendingString: vmapUrlQueryParameters];
                [updatedJsonResponseSources addObject:mutableSrc];
            }
            
            updatedJsonResponseDict[@"sources"] = updatedJsonResponseSources;
            BCOVVideo *video = [BCOVPlaybackService videoFromJSONDictionary:updatedJsonResponseDict];
